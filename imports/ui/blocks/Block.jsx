  ...
  render() {
    if (this.props.loading) {
      ...
      if (this.props.blockExist) {
        ...
        return (
          <Container id="block" className="paddingleft">
            <Helmet>
              <title>
                Block {numbro(block.height).format("0,0")} on Color Explorer |
                Color
              </title>
              <meta
                name="description"
                content={
                  "Block details of height " +
                  numbro(block.height).format("0,0")
                }
              />
            </Helmet>
            <h4>
              <T>blocks.block</T> {numbro(block.height).format("0,0")}
            </h4>
            <Card>
              ...
              <CardBody>
                <Row>
                  ...
                  <Col md={8} className="value address">
                    {block.hash}
                  </Col>
                  <Col md={4} className="label">
                    <T>blocks.proposer</T>
                  ...
                  <Col md={8} className="value">
                    <Link
                      to={"/validator/" + (proposer ? proposer.address : "")}
                    >
                      <Avatar
                        moniker={moniker}
                        identity={identity}
                  ...
                  <Col md={8} className="value">
                    {moment.utc(block.time).format("D MMM YYYY, h:mm:ssa z")} (
