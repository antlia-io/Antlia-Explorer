...
export default BlockContainer = withTracker(props => {
  ...
  if (Meteor.isClient) {
    blockHandle = Meteor.subscribe(
      "blocks.findOne",
      parseInt(props.block.height)
    );
    transactionHandle = Meteor.subscribe(
      "transactions.height",
      parseInt(props.block.height)
    );
  ...
  if (Meteor.isServer || !loading) {
    block = Blockscon.findOne({ height: parseInt(props.block.height) });
    txs = Transactions.find({ height: parseInt(props.block.height) });
  ...
  return {
    ...
    stakingTxs: transactionsExist
      ...
      : {},
    distributionTxs: transactionsExist
      ? Transactions.find({
          $or: [
            {
      ...
      : {},
    governanceTxs: transactionsExist
      ? Transactions.find({
          $or: [
      ...
      : {},
    slashingTxs: transactionsExist
      ? Transactions.find({
          $or: [{ "tx.value.msg.type": "cosmos-sdk/MsgUnjail" }]
        }).fetch()
      : {},
    IBCTxs: transactionsExist
      ? Transactions.find({
          $or: [
