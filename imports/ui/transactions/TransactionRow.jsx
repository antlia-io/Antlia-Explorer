...
export const TransactionRow = (props) => {
    ...
    return <Row className={(!props.blockList) ? "tx-info tx-row" : "bb"}>
        {(!props.blockList) ? <Col xs={12} sm={6} md={6} lg={2}>{(tx.tx.value.msg && tx.tx.value.msg.length > 0) ? tx.tx.value.msg.map((msg) => {
        ...
        <Col className="resultpaddingleft" xs={12} sm={6} md={6} lg={(!props.blockList) ? 1 : 4}>{(!tx.code) ? <TxIcon valid /> : <TxIcon />}</Col>
        ...
        <Col xs={12} sm={6} md={6} lg={(!props.blockList) ? 2 : 4} className="fee textalign">
